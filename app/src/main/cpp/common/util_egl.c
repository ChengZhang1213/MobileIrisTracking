...
int
egl_init_with_window_surface(int gles_version, void *window, int depth_size, int stencil_size,
                             int sample_num) {
    EGLint major, minor;
    EGLConfig config;
    EGLBoolean ret;
...
int egl_init_with_native_window(EGLNativeWindowType  win){
    EGLint major, minor;
    EGLConfig config;
    EGLBoolean ret;
    EGLint context_attribs[] =
    ...
    EGLint num_config;
    ret = eglChooseConfig(s_dpy, attribs, NULL, 1, &num_config);
//    config = find_egl_config(8, 8, 8, 8, 8, 8, 8, EGL_RENDERABLE_TYPE,
//                             EGL_OPENGL_ES2_BIT);
    if (ret != EGL_TRUE) {
        fprintf(stderr, "ERR: %s(%d)\n", __FILE__, __LINE__);
        return -1;
    }
   ret =  eglChooseConfig(s_dpy, attribs, &config, num_config, &num_config);
    if (ret != EGL_TRUE) {
        fprintf(stderr, "ERR: %s(%d)\n", __FILE__, __LINE__);
        return -1;
    }
    ...
    s_sfc = eglCreateWindowSurface(s_dpy, config, win, NULL);
    if (s_sfc == EGL_NO_SURFACE) {
        fprintf(stderr, "ERR: %s(%d)\n", __FILE__, __LINE__);
        return (-1);
    }
...
egl_show_current_context_attrib() {
    ...
    fprintf(stderr, " %-32s: ", "EGL_CONFIG_ID");
    ival = -1;
    eglQueryContext(dpy, ctx, EGL_CONFIG_ID, &ival);
    fprintf(stderr, "%d\n", ival);

    fprintf(stderr, " %-32s: ", "EGL_CONTEXT_CLIENT_TYPE");
    ival = -1;
    eglQueryContext(dpy, ctx, EGL_CONTEXT_CLIENT_TYPE, &ival);
    switch (ival) {
        case EGL_OPENGL_API   :
            fprintf(stderr, "EGL_OPENGL_API\n");
            break;
        case EGL_OPENGL_ES_API:
            fprintf(stderr, "EGL_OPENGL_ES_API\n");
            break;
        case EGL_OPENVG_API   :
            fprintf(stderr, "EGL_OPENVG_API\n");
            break;
        default:
    ...
    fprintf(stderr, " %-32s: ", "EGL_CONTEXT_CLIENT_VERSION");
    ival = -1;
    eglQueryContext(dpy, ctx, EGL_CONTEXT_CLIENT_VERSION, &ival);
    fprintf(stderr, "%d\n", ival);

    fprintf(stderr, " %-32s: ", "EGL_RENDER_BUFFER");
    ival = -1;
...
egl_show_current_config_attrib() {
    ...
    fprintf(stderr, " %-32s: ", "EGL_CONFIG_CAVEAT");
    ival = -1;
    ...
    fprintf(stderr, " %-32s: ", "EGL_CONFORMANT");
    ival = -1;
    ...
    fprintf(stderr, " %-32s: ", "EGL_RENDERABLE_TYPE");
    ival = -1;
    ...
    fprintf(stderr, " %-32s: ", "EGL_TRANSPARENT_TYPE");
    ival = -1;
    ...
    fprintf(stderr, " %-32s: ", "EGL_TRANSPARENT_RED_VALUE");
    ival = -1;
    eglGetConfigAttrib(dpy, cfg, EGL_TRANSPARENT_RED_VALUE, &ival);
    fprintf(stderr, "%d\n", ival);

    fprintf(stderr, " %-32s: ", "EGL_TRANSPARENT_GREEN_VALUE");
    ival = -1;
    eglGetConfigAttrib(dpy, cfg, EGL_TRANSPARENT_GREEN_VALUE, &ival);
    fprintf(stderr, "%d\n", ival);

    fprintf(stderr, " %-32s: ", "EGL_TRANSPARENT_BLUE_VALUE");
    ival = -1;
...
egl_show_current_surface_attrib() {
    ...
    fprintf(stderr, " %-32s: ", "EGL_SWAP_BEHAVIOR");
    ival = -1;
    ...
    fprintf(stderr, " %-32s: ", "EGL_HORIZONTAL_RESOLUTION");
    ival = -1;
    eglQuerySurface(dpy, sfc, EGL_HORIZONTAL_RESOLUTION, &ival);
    fprintf(stderr, "%d\n", ival);

    fprintf(stderr, " %-32s: ", "EGL_VERTICAL_RESOLUTION");
    ival = -1;
    eglQuerySurface(dpy, sfc, EGL_VERTICAL_RESOLUTION, &ival);
    fprintf(stderr, "%d\n", ival);

    fprintf(stderr, " %-32s: ", "EGL_PIXEL_ASPECT_RATIO");
    ival = -1;
